...
APP_VERSION = '0.5 Alpha - Commit 0001u1'
THEME = {
    'primary': '#005DAA',
    'primary_dark': '#07355F',
    'primary_mid': '#0079C2',
...
class LoginDialog(tk.Toplevel):
    def __init__(self, master, data):
        super().__init__(master); self.data=data; self.result=None; self.title('Startup Selection'); self.geometry('560x440'); self.resizable(False,False); self.configure(bg='#f3f8fd'); self.transient(master); self.grab_set(); self.lift(); self.attributes('-topmost', True); self.after(300, lambda: self.attributes('-topmost', False)); self.focus_force()
...
class Hub(tk.Tk):
    def __init__(self):
        ...
        self.login(); self.language_var=tk.StringVar(value=self.current_language()); self.output_var=tk.StringVar(value=self.data.get_default(self.session,'default_output_mode','Copy Template')); self.build()
    ...
    def card(self,parent,key,r,c,admin=False):
        ...
        f=tk.Frame(parent,bg='white',highlightbackground=THEME['line'],highlightthickness=1,width=320,height=150); f.grid(row=r,column=c,sticky='nsew',padx=10,pady=10); f.grid_propagate(False)
        parent.columnconfigure(c,weight=1); parent.rowconfigure(r,weight=1)
        stripe=tk.Frame(f,bg=color,height=5); stripe.pack(fill='x')
        tk.Label(f,text=title,bg='white',fg=color,font=('Segoe UI',14,'bold')).pack(anchor='w',padx=18,pady=(14,5))
        tk.Label(f,text=desc,bg='white',fg=THEME['text'],wraplength=250,justify='left',font=('Segoe UI',10)).pack(anchor='w',padx=18,pady=2)
        ...
        tk.Label(f,text=badge,bg=THEME['surface_2'],fg=THEME['primary_dark'],font=('Segoe UI',9,'bold'),padx=8,pady=3).pack(anchor='w',padx=18,pady=(4,0))
        tk.Button(f,text='Start  ›',bg=color,fg='white',activebackground=color,activeforeground='white',font=('Segoe UI',11,'bold'),relief='flat',command=lambda:self.open_feature(key)).pack(fill='x',side='bottom',padx=18,pady=12,ipady=5)
